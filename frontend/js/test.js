'use strict';
// let developmentConfig = require('../../api/config/development.config.js')

// $(function(){
//
//   $('#credentials-test').click(function(event){
//     event.preventDefault();
//
//     console.log('clicked credentials-test button');
//
//     $.ajax({
//       // hard-coded route to test client_id, client_secret and other params
//       url: 'https://api.foursquare.com/v2/venues/search?client_id=' + client_id + '&client_secret=' + client_secret + '&ll=40.7,-74&query=sushi&v=20140806&m=foursquare'
//     }).done(function(data) {
//       console.log(data)
//     })
//   })
//
//   $('#foursquare-auth').click(function(event){
//     event.preventDefault();
//     console.log('clicked foursquare auth button');
//     $.ajax({
//       url: '/auth-test'
//     })
//     // $.ajax({
//     //   url: 'https://foursquare.com/oauth2/authenticate?client_id=' + client_id + '&response_type=code&redirect_uri=https://graffiti-hunt.herokuapp.com/'
//     // })
//   })
//
// })
